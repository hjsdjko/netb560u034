...
namespace Xiezn.Core.Business.DB
{
    public class DbContext<T> where T : class, new()
    {
        ...
        public dynamic Common(string tableName, string columnName, string columnValue = "", int type = 0, string sign = "option", int remindStart = 0, int remindEnd = 0, string mywhere = "", bool ri = true)
        {
            DbConfig dbConfig = ConfigHelper.GetConfig<DbConfig>("DbConfig");
            sign = sign.ToLower();
            string sql = "";
                ...
                case "remind":
                    ...
                    if (type == 2)
                    {
                        ...
                        if(ConfigHelper.GetConfig<DbConfig>("DbConfig").DbType.ToLower() == "mysql")
                        {
                            if (ri)
                            {
                                dtStartStr = dtStart.ToString("yyyy-MM-dd");
                                dtEndStr = dtEnd.ToString("yyyy-MM-dd");
                            }
                            else {
